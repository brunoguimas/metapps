...
func NewModule(repo user.UserRepository, users user.UserService, tokens jwt.JWTService, emails mail.EmailService, profiles profile.ProfileService, c *config.Config) *Module {
	service := NewAuthService(repo, profiles)
