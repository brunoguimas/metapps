...
export default function Homepage() {
  ...
  const [view, setView] = useState('home') // home | roadmap | phase | task | result | profile
  ...
  const [selPhase, setSelPhase] = useState(null) // fase aberta na view 'phase'
      ...
      <div style={page}>
        ...
        <div style={header}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={() => setView('profile')} style={iconNavBtn} aria-label="Perfil"><IconUser /></button>
            <button type="button" onClick={() => navigate('/history')} style={iconNavBtn} aria-label="Histórico"><IconHistory /></button>
            <button type="button" onClick={handleLogout} style={iconNavBtn} aria-label="Sair"><IconLogout /></button>
          </div>
        </div>
        ...
        <div style={box}>
          {username && (
            <div style={welcomeText}>
              Bem-vindo(a) de volta, <b style={{ color: '#1E1E32' }}>{username}</b>
            </div>
          )}
          ...
          <div style={{ ...card, marginTop: 14, padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '0 20px 20px' }}>
              ...
              <button type="button" onClick={handleSend} disabled={!input.trim() || loading} style={{ ...btn, opacity: (!input.trim() || loading) ? 0.6 : 1 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {goals.slice(0, 8).map(g => (
                      <div key={g.id} style={{ display: 'flex', gap: 8 }}>
                        <button type="button" onClick={() => handleOpenGoal(g)} style={{ ...goalBtn, flex: 1 }}>
                          <span style={goalIconWrap}><MountainMark size={14} color="#6382FF" /></span>
                          <span style={{ flex: 1 }}>{g.title}</span>
                          <IconChevron />
                        </button>
                        <button type="button" onClick={() => { setEditingGoal(g); setEditInput(g.title) }} style={squareIconBtn} aria-label="Editar">
                          <IconEdit />
                        </button>
                        <button type="button" onClick={() => setDeletingGoal(g)} style={{ ...squareIconBtn, color: '#F06A6A' }} aria-label="Excluir">
                          <IconTrash />
                        </button>
                      </div>
                    ))}
          ...
          {editingGoal && (
            <div style={overlay} onClick={() => setEditingGoal(null)}>
              <div style={modal} onClick={e => e.stopPropagation()}>
                ...
                <textarea value={editInput} onChange={e => setEditInput(e.target.value)} rows={2} autoFocus style={ta} />
                {err && <div style={errStyle}><IconAlert /> {err}</div>}
                <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                  <button type="button" onClick={() => setEditingGoal(null)} style={btnGhost}>Cancelar</button>
                  <button type="button" onClick={handleUpdateGoal} disabled={!editInput.trim() || loading} style={{ ...btn, flex: 1, marginTop: 0, opacity: (!editInput.trim() || loading) ? 0.6 : 1 }}>
          ...
          {deletingGoal && (
            <div style={overlay} onClick={() => setDeletingGoal(null)}>
              <div style={modal} onClick={e => e.stopPropagation()}>
                ...
                {err && <div style={errStyle}><IconAlert /> {err}</div>}
                <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                  <button type="button" onClick={() => setDeletingGoal(null)} style={btnGhost}>Cancelar</button>
                  <button type="button" onClick={() => handleDeleteGoal(deletingGoal)} disabled={loading} style={{ ...btn, flex: 1, marginTop: 0, background: '#F06A6A', boxShadow: '0 8px 20px rgba(240,106,106,0.35)', opacity: loading ? 0.6 : 1 }}>
  ...
  if (view === 'roadmap') {
    ...
    const safeTotal = Math.max(1, totalLeaves)
    const progressPct = Math.min(100, Math.round((doneLeaves / safeTotal) * 100))
    ...
    return (
      <div style={mountainPage}>
        ...
        <div style={mountainHeader}>
          <button type="button" onClick={() => setView('home')} style={iconNavBtn} aria-label="Início"><IconArrowLeft /></button>
        ...
        <div style={{ maxWidth: 560, margin: '0 auto', padding: '10px 16px 12px', position: 'relative', zIndex: 5 }}>
          ...
          <div style={hudBarTrack}>
            <div style={{ ...hudBarFill, width: `${progressPct}%` }} />
                ...
                return (
                  <div key={ph.key} style={{ position: 'absolute', left: `${a.x}%`, top: a.y, transform: `translate(-50%,-100%) scale(${a.scale})`, transformOrigin: '50% 100%', zIndex: 3 }}>
                    <StageLandmark
                      ...
                      onClick={() => {
                        if (ph.locked) return
                        setSelPhase({ ...ph, theme: THEMES[themeIdx] })
                        setView('phase')
                      }}
                      disabled={ph.locked}
                    />
  ...
  if (view === 'phase' && selPhase) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: theme.bg, color: '#F5F4FF', fontFamily: "'Inter',-apple-system,sans-serif", WebkitFontSmoothing: 'antialiased', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={mountainHeader}>
          <button type="button" onClick={() => setView('roadmap')} style={iconNavBtn} aria-label="Voltar"><IconArrowLeft /></button>
          ...
          <button type="button" onClick={() => setView('home')} style={iconNavBtn} aria-label="Início"><IconHome /></button>
        </div>
        ...
        <div style={box}>
          ...
          {isQuiz &&
            questions.map((q, qi) => (
              <div key={qi} style={questionCard}>
                {(typeof q.material === 'string' ? Boolean(q.material) : textMaterials(q.material).length > 0) && (
                  <div style={materialTag}>MATERIAL</div>
                )}
                {textMaterials(q.material).map((t, mi) => (
                  <p key={mi} style={materialText}>{t}</p>
                ))}
                <p style={{ color: '#1E1E32', fontSize: 15, marginBottom: 14, lineHeight: 1.6, fontWeight: 600 }}>
                  <span style={{ color: '#6382FF' }}>{qi + 1}.</span> {q.statement || q.question}
                </p>
                {sa(q.options || q.alternatives).map((opt, ai) => {
                  const sel = answers[qi] === ai
                  return (
                    <button type="button" key={ai} onClick={() => setAnswers(p => ({ ...p, [qi]: ai }))} style={{ ...optionBtn, ...(sel ? optionBtnSel : {}) }}>
                      <span style={{ ...optionKey, ...(sel ? optionKeySel : {}) }}>{String.fromCharCode(65 + ai)}</span>
                      {opt}
                    </button>
                  )
                })}
          ...
          {!isQuiz && (
            <div style={questionCard}>
              {textMaterials(content.material).map((t, mi) => (
                <p key={mi} style={materialText}>{t}</p>
              ))}
              ...
              <textarea value={essay} onChange={e => { setEssay(e.target.value); setErr('') }} placeholder="Escreva sua resposta aqui…" rows={12} style={{ ...ta, marginBottom: 6 }} />
              <p style={{ color: '#8A8AA3', fontSize: 12, fontWeight: 700, textAlign: 'right' }}>
          ...
          {err && <div style={errStyle}><IconAlert /> {err}</div>}
          <button type="button" onClick={handleSubmit} disabled={loading} style={{ ...btn, opacity: loading ? 0.6 : 1 }}>
  ...
  if (view === 'result' && result) {
    ...
    return (
      <div style={page}>
        <GlobalStyles />
        <div style={header}>
          <button type="button" onClick={() => setView('roadmap')} style={iconNavBtn} aria-label="Roadmap"><IconArrowLeft /></button>
        ...
        <div style={box}>
          <div style={{ ...resultCard, ...(mastered ? { boxShadow: `0 0 0 1px ${mc}33, 0 16px 40px ${mc}22` } : {}) }}>
            ...
            <svg width="128" height="128" viewBox="0 0 128 128" style={{ position: 'relative' }}>
              ...
              <circle cx="64" cy="64" r="52" fill="none" stroke={mc} strokeWidth="10" strokeLinecap="round"
                strokeDasharray={ringCirc} strokeDashoffset={ringCirc - (ringCirc * pct / 100)} transform="rotate(-90 64 64)" style={{ transition: 'stroke-dashoffset .8s ease' }} />
            ...
            <div style={{ color: '#1E1E32', fontSize: 16, fontWeight: 800, marginTop: 10, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
            <button type="button" onClick={() => { setView('task'); setAnswers({}); setEssay(''); setResult(null); setErr('') }} style={btnGhost}>
              Tentar novamente
            </button>
            <button type="button" onClick={() => setView('roadmap')} style={{ ...btn, flex: 1, marginTop: 0 }}>
        ...
        <div style={box}>
          <div style={{ ...card, textAlign: 'center', paddingTop: 28 }}>
            ...
            <div style={{ marginTop: 20, textAlign: 'left' }}>
              ...
              <div style={{ fontSize: 11, color: '#8A8AA3', marginTop: 6, textAlign: 'right', fontWeight: 700 }}>
                {Math.max(0, xpNext - xpInLevel)} XP para o nível {level + 1}
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginTop: 16 }}>
            {[
              { l: 'Nível', v: level, c: '#6382FF', bg: 'rgba(99,130,255,0.1)' },
              { l: 'XP total', v: xp, c: '#F5C542', bg: 'rgba(245,197,66,0.14)' },
              { l: 'Próximo nível', v: `${Math.max(0, xpNext - xpInLevel)} XP`, c: '#3ECF8E', bg: 'rgba(62,207,142,0.12)' },
            ].map(({ l, v, c, bg }) => (
...
function StageLandmark({ theme, state, title, count, onClick, disabled }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '190px' }}>
      <button type="button" onClick={onClick} disabled={disabled} aria-label={title}
...
function LessonRow({ n, done, current, onClick, accent }) {
  return (
    <button type="button" onClick={onClick} disabled={n.blocked}
...
function IconTrash({ size = 14 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18" /><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /></svg>
}
function IconLock({ size = 14 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="11" width="16" height="9" rx="2" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg>
}
...
function IconX({ size = 16, color = 'currentColor' }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18" /><path d="M6 6l12 12" /></svg>
}
...
function IconArrowRight() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: '-3px', marginLeft: 6 }}><path d="M5 12h14" /><path d="M12 5l7 7-7 7" /></svg>
}
function IconChevron({ color = '#8A8AA3' }) {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg>
}
function IconUser() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4" /><path d="M4 21c0-4 4-6 8-6s8 2 8 6" /></svg>
}
function IconHome() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 10.5 12 3l9 7.5" /><path d="M5 9.5V21h14V9.5" /><path d="M9.5 21v-6h5v6" /></svg>
}
function IconLogout() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><path d="M16 17l5-5-5-5" /><path d="M21 12H9" /></svg>
}
function IconHistory() {
  return <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><path d="M12 7v5l3 2" /></svg>
}
function IconFlag() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F5C542" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22V4" /><path d="M4 4h14l-3 4 3 4H4" /></svg>
}
function IconAlert() {
...
const materialText = { color: '#4A4A66', fontSize: 13, fontStyle: 'italic', lineHeight: 1.6, margin: '0 0 12px', padding: '10px 12px', background: '#F5F4FF', borderRadius: 10, borderLeft: '3px solid #6382FF' }
